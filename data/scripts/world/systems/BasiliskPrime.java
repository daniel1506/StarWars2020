...
public class BasiliskPrime {

	public void generate(SectorAPI sector) {
		
		StarSystemAPI system = sector.createStarSystem("Basilisk");
		LocationAPI hyper = Global.getSector().getHyperspace();
		
		system.setBackgroundTextureFilename("graphics/backgrounds/background5.jpg");
		system.getLocation().set(2733.2f, 14349.3f);
		
		SectorEntityToken basilisk_nebula = Misc.addNebulaFromPNG("data/campaign/terrain/valhalla_nebula.png",
		...
		PlanetAPI basilisk_star = system.initStar("basilisk_prime", // unique id for this star 
		...
		PlanetAPI basilisk1 = system.addPlanet("basilisk", basilisk_star, "Basilisk", "barren", 30, 100, 5000, 360);
		basilisk1.getSpec().setPlanetColor(new Color(205,205,205,205));
		basilisk1.setCustomDescriptionId("planet_basilisk");
		basilisk1.applySpecChanges();
		...
		MarketAPI basilisk_market = SW_AddMarket.SW_AddMarket("sw_mandalorian", (SectorEntityToken)basilisk1, null, "Basilisk", 7, new ArrayList<String>(Arrays.asList("population_7", "toxic_atmosphere", "pollution", "barren_marginal", "headquarters", "organics_plentiful", "volatiles_plentiful")), new ArrayList<String>(Arrays.asList("population", "spaceport", "militarybase", "mining", "fuelprod", "heavyindustry", "heavybatteries", "sw_golan_iii_mandal", "patrolhq")), new ArrayList<String>(Arrays.asList("storage", "black_market", "open_market", "generic_military")), 0.2f);
		
		//SpecialItemData nanoforge = new SpecialItemData("pristine_nanoforge", "special_item1");
		basilisk_market.getIndustry("militarybase").setAICoreId("alpha_core");
		basilisk_market.getIndustry("mining").setAICoreId("alpha_core");
		basilisk_market.getIndustry("fuelprod").setAICoreId("alpha_core");
		basilisk_market.getIndustry("patrolhq").setAICoreId("alpha_core");
		basilisk_market.getIndustry("sw_golan_iii_mandal").setAICoreId("alpha_core");
		basilisk_market.getIndustry("population").setAICoreId("beta_core");
		basilisk_market.getIndustry("spaceport").setAICoreId("beta_core");
		basilisk_market.getIndustry("heavybatteries").setAICoreId("beta_core");
		basilisk_market.getIndustry("heavyindustry").setAICoreId("beta_core");
		...
		// jump point 
		JumpPointAPI jumpPoint = Global.getFactory().createJumpPoint("basilisk_inner_jump", "Basilisk Inner System Jump-point");
		OrbitAPI orbit = Global.getFactory().createCircularOrbit(basilisk1, 0, 1500, 65);
		jumpPoint.setOrbit(orbit);
		jumpPoint.setRelatedPlanet(basilisk1);
