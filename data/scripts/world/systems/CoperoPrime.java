...
public class CoperoPrime {

	public void generate(SectorAPI sector) {
		
		StarSystemAPI system = sector.createStarSystem("Copero");
		LocationAPI hyper = Global.getSector().getHyperspace();
		
		system.setBackgroundTextureFilename("graphics/backgrounds/background5.jpg");
		system.getLocation().set(-46464.4f, 19815.7f);
		//Star Wars map to Starsector map ratio: 1x1 (grid) : 6833x4727
		
		SectorEntityToken copero_nebula = Misc.addNebulaFromPNG("data/campaign/terrain/valhalla_nebula.png",
		...
		PlanetAPI copero_star = system.initStar("copero_prime", // unique id for this star 
		...
		PlanetAPI copero1 = system.addPlanet("copero", copero_star, "Copero", "terran", 24, 120f, 5000, 360);
		copero1.getSpec().setPlanetColor(new Color(205,205,205,205));
		copero1.setCustomDescriptionId("planet_copero");
		copero1.applySpecChanges();
		...
		MarketAPI copero_market = SW_AddMarket.SW_AddMarket("sw_chiss", (SectorEntityToken)copero1, null, "Copero", 9, new ArrayList<String>(Arrays.asList("population_9", "habitable", "mild_climate", "industrial_polity", "ore_rich", "rare_ore_rich")), new ArrayList<String>(Arrays.asList("population", "megaport", "waystation", "militarybase", "orbitalworks", "mining", "refining", "heavybatteries", "sw_shipyards_chiss")), new ArrayList<String>(Arrays.asList("storage", "black_market", "open_market", "generic_military")), 0.2f);
		...
		copero_market.getIndustry("militarybase").setAICoreId("alpha_core");
		copero_market.getIndustry("orbitalworks").setAICoreId("alpha_core");		
		copero_market.getIndustry("mining").setAICoreId("alpha_core");
		copero_market.getIndustry("refining").setAICoreId("alpha_core");
		copero_market.getIndustry("sw_shipyards_chiss").setAICoreId("alpha_core");
		copero_market.getIndustry("population").setAICoreId("beta_core");
		copero_market.getIndustry("megaport").setAICoreId("beta_core");
		copero_market.getIndustry("heavybatteries").setAICoreId("beta_core");
		copero_market.getIndustry("waystation").setAICoreId("alpha_core");
		...
		// jump point 
		JumpPointAPI jumpPoint = Global.getFactory().createJumpPoint("copero_inner_jump", "Copero Inner System Jump-point");
		OrbitAPI orbit = Global.getFactory().createCircularOrbit(copero1, 0, 1500, 65);
		jumpPoint.setOrbit(orbit);
		jumpPoint.setRelatedPlanet(copero1);
