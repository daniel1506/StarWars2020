...
public class LothalPrime {

	public void generate(SectorAPI sector) {
		
		StarSystemAPI system = sector.createStarSystem("Lothal");
		LocationAPI hyper = Global.getSector().getHyperspace();
		
		system.setBackgroundTextureFilename("graphics/backgrounds/background5.jpg");
		system.getLocation().set(56713.9f, 23162.3f);
		
		SectorEntityToken lothal_nebula = Misc.addNebulaFromPNG("data/campaign/terrain/valhalla_nebula.png",
		...
		PlanetAPI lothal_star = system.initStar("lothal_prime", // unique id for this star 
		...
		PlanetAPI lothal1 = system.addPlanet("lothal", lothal_star, "Lothal", "terran", 24, 120f, 5000, 210);
		lothal1.getSpec().setPlanetColor(new Color(205,205,205,205));
		lothal1.setCustomDescriptionId("planet_lothall");
		lothal1.applySpecChanges();
		...
		MarketAPI lothal_market = SW_AddMarket.SW_AddMarket("sw_first_order", (SectorEntityToken)lothal1, null, "Lothal", 8, new ArrayList<String>(Arrays.asList("population_8", "terran", "habitable", "mild_climate", "urbanized_polity", "farmland_rich")), new ArrayList<String>(Arrays.asList("population", "spaceport", "farming", "patrolhq", "lightindustry", "heavyindustry", "commerce", "heavybatteries", "sw_golan_iii_firstorder")), new ArrayList<String>(Arrays.asList("storage", "black_market", "open_market", "generic_military")), 0.3f);
		...
		lothal_market.getIndustry("farming").setAICoreId("alpha_core");
		lothal_market.getIndustry("lightindustry").setAICoreId("alpha_core");
		lothal_market.getIndustry("heavyindustry").setAICoreId("alpha_core");
		lothal_market.getIndustry("commerce").setAICoreId("alpha_core");
		lothal_market.getIndustry("patrolhq").setAICoreId("alpha_core");
		lothal_market.getIndustry("sw_golan_iii_firstorder").setAICoreId("alpha_core");
		lothal_market.getIndustry("population").setAICoreId("beta_core");
		lothal_market.getIndustry("spaceport").setAICoreId("beta_core");
		lothal_market.getIndustry("heavybatteries").setAICoreId("beta_core");		
		...
		// jump point 
		JumpPointAPI jumpPoint = Global.getFactory().createJumpPoint("lothal_inner_jump", "Lothal Inner System Jump-point");
		OrbitAPI orbit = Global.getFactory().createCircularOrbit(lothal1, 0, 1500, 65);
		jumpPoint.setOrbit(orbit);
		jumpPoint.setRelatedPlanet(lothal1);
