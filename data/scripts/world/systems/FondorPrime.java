...
public class FondorPrime {

	public void generate(SectorAPI sector) {
		
		StarSystemAPI system = sector.createStarSystem("Fondor");
		LocationAPI hyper = Global.getSector().getHyperspace();
		
		system.setBackgroundTextureFilename("graphics/backgrounds/background5.jpg");
		system.getLocation().set(4783.1f, 11616.1f);
		//system.getLocation().set(6149.7f, 7090.5f);
		//Star Wars map to Starsector map ratio: 1x1 (grid) : 6833x4727
		
		SectorEntityToken fondor_nebula = Misc.addNebulaFromPNG("data/campaign/terrain/valhalla_nebula.png",
		...
		PlanetAPI fondor_star = system.initStar("fondor_prime", // unique id for this star 
		...
		PlanetAPI fondor1 = system.addPlanet("fondor", fondor_star, "Fondor", "ecumenopolis", 24, 91f, 5000, 412);
		fondor1.getSpec().setPlanetColor(new Color(205,205,205,205));
		fondor1.setCustomDescriptionId("planet_fondor");
		fondor1.applySpecChanges();
		...
		MarketAPI fondor_market = SW_AddMarket.SW_AddMarket("sw_republic", (SectorEntityToken)fondor1, null, "Fondor", 9, new ArrayList<String>(Arrays.asList("population_9", "habitable", "mild_climate", "sw_ecumenopolis", "urbanized_polity", "industrial_polity", "trade_center")), new ArrayList<String>(Arrays.asList("population", "megaport", "militarybase", "commerce", "lightindustry", "heavyindustry", "patrolhq", "heavybatteries", "sw_golan_iii_republic")), new ArrayList<String>(Arrays.asList("storage", "black_market", "open_market", "generic_military")), 0.2f);
		
		MarketAPI fondor_shipyards_market = SW_AddMarket.SW_AddMarket("sw_republic", (SectorEntityToken)fondorStation, null, "Fondor Shipyard", 8, new ArrayList<String>(Arrays.asList("population_8", "no_atmosphere", "outpost", "industrial_polity")), new ArrayList<String>(Arrays.asList("population", "spaceport", "militarybase", "lightindustry", "orbitalworks", "waystation", "heavybatteries", "sw_shipyards_republic")), new ArrayList<String>(Arrays.asList("storage", "black_market", "open_market", "generic_military")), 0.3f);				
		
		SpecialItemData nanoforge = new SpecialItemData("pristine_nanoforge", "special_item1");
		fondor_shipyards_market.getIndustry("orbitalworks").setSpecialItem(nanoforge);
		fondor_shipyards_market.getIndustry("militarybase").setAICoreId("alpha_core");
		fondor_shipyards_market.getIndustry("lightindustry").setAICoreId("alpha_core");		
		fondor_shipyards_market.getIndustry("waystation").setAICoreId("alpha_core");
		fondor_shipyards_market.getIndustry("sw_shipyards_republic").setAICoreId("alpha_core");
		fondor_shipyards_market.getIndustry("population").setAICoreId("beta_core");
		fondor_shipyards_market.getIndustry("spaceport").setAICoreId("beta_core");
		fondor_shipyards_market.getIndustry("heavybatteries").setAICoreId("beta_core");
		
		fondor_market.getIndustry("militarybase").setAICoreId("alpha_core");
		fondor_market.getIndustry("commerce").setAICoreId("alpha_core");		
		fondor_market.getIndustry("lightindustry").setAICoreId("alpha_core");
		fondor_market.getIndustry("heavyindustry").setAICoreId("alpha_core");
		fondor_market.getIndustry("patrolhq").setAICoreId("alpha_core");
		fondor_market.getIndustry("sw_golan_iii_republic").setAICoreId("alpha_core");
		fondor_market.getIndustry("population").setAICoreId("beta_core");
		fondor_market.getIndustry("megaport").setAICoreId("beta_core");
		fondor_market.getIndustry("heavybatteries").setAICoreId("beta_core");
		...
		// jump point 
		JumpPointAPI jumpPoint = Global.getFactory().createJumpPoint("fondor_inner_jump", "Fondor Inner System Jump-point");
		OrbitAPI orbit = Global.getFactory().createCircularOrbit(fondor1, 0, 1500, 65);
		jumpPoint.setOrbit(orbit);
		jumpPoint.setRelatedPlanet(fondor1);
