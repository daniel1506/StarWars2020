...
public class RequestEscortRepublicStats extends BaseShipSystemScript {
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		if (engine.getPlayerShip().getOriginalOwner() == ship.getOriginalOwner() && alreadyExecuted == false) {
			...
			if (ship.getHullSize() == HullSize.CAPITAL_SHIP) {
				ShipAPI escort1 = FleetManager.spawnShipOrWing("sw_pelta_reinforcement_republic", jumpPoint1, 90f, 1.5f);
				ShipAPI escort2 = FleetManager.spawnShipOrWing("sw_pelta_reinforcement_republic", jumpPoint2, 90f, 1.5f);
				...
				ShipAPI escort1 = FleetManager.spawnShipOrWing("sw_consular_reinforcement_republic", jumpPoint1, 90f, 1.5f);
				task.giveAssignment(FleetManager.getDeployedFleetMember(escort1), task.createAssignment(CombatAssignmentType.LIGHT_ESCORT, target, false), false);	
			}			
		}
			...
			jumpPoint2.set(ship.getLocation().getX() + ship.getShieldRadiusEvenIfNoShield() + 70f, ship.getLocation().getY() - ship.getShieldRadiusEvenIfNoShield() - 70f);
			if (ship.getHullSize() == HullSize.CAPITAL_SHIP) {
				ShipAPI escort1 = FleetManager.spawnShipOrWing("sw_pelta_reinforcement_republic", jumpPoint1, 270f, 1.5f);
				ShipAPI escort2 = FleetManager.spawnShipOrWing("sw_pelta_reinforcement_republic", jumpPoint2, 270f, 1.5f);
				task.giveAssignment(FleetManager.getDeployedFleetMember(escort1), task.createAssignment(CombatAssignmentType.LIGHT_ESCORT, target, false), false);
				task.giveAssignment(FleetManager.getDeployedFleetMember(escort2), task.createAssignment(CombatAssignmentType.LIGHT_ESCORT, target, false), false);
			}
			else {
				ShipAPI escort1 = FleetManager.spawnShipOrWing("sw_consular_reinforcement_republic", jumpPoint1, 270f, 1.5f);
				task.giveAssignment(FleetManager.getDeployedFleetMember(escort1), task.createAssignment(CombatAssignmentType.LIGHT_ESCORT, target, false), false);	
			}
		}		
