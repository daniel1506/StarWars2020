package data.missions.sw_battle_of_scarif;
...
public class MissionDefinition implements MissionDefinitionPlugin
{
    public void defineMission(final MissionDefinitionAPI api) {
        ...
        api.setFleetTagline(FleetSide.ENEMY, "Galactic Empire Patrol Fleet");
        api.addBriefingItem("Destroy the shield gate!");
		...
		api.addToFleet(FleetSide.PLAYER, "sw_ywing_wing", FleetMemberType.FIGHTER_WING, "Gold Squadron", false);
		api.addToFleet(FleetSide.PLAYER, "sw_ywing_wing", FleetMemberType.FIGHTER_WING, "Gold Squadron", false);
        api.addToFleet(FleetSide.ENEMY, "sw_imperial_i_imperial", FleetMemberType.SHIP, "Intimidator", true);
		api.addToFleet(FleetSide.ENEMY, "sw_imperial_i_imperial", FleetMemberType.SHIP, "Persecutor", false);
        api.addToFleet(FleetSide.ENEMY, "sw_station_shieldgate_imperial", FleetMemberType.SHIP, "Golan M3185 Shield Gate", false);
		api.defeatOnShipLoss("Golan M3185 Shield Gate");
        final float width = 10000.0f;
        final float height = 10000.0f;
        api.initMap(-width / 2.0f, width / 2.0f, -height / 2.0f, height / 2.0f);
		api.addPlanet(0f, 0f, 2500f, "terran", 1f, false);
		api.setPlanetBgSize(2500f, 2500f);
